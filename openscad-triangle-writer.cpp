 ...
 * (with invalid 'normals' value - but these are ignored anyhow in OpenSCAD
 */
...
/* Write every faces (i.e. all trianges of each face) into a separate
 * points/faces vector pairs.
 *
...
void write_faces_scad(const std::vector<Face>& faces) {
  ...
  for(i=1; i<=faces.size(); i++) {
     auto color = colors[i % NUM_COLORS];
     std::cout << "color(\"" << color << "\")" << std::endl;
     std::cout << "polyhedron(face_" << i <<"_points, face_" << i << "_faces);"
               << std::endl;
     }
  std::cout << "}" << std::endl;
